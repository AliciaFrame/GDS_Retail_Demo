...
CALL gds.graph.create.cypher('item_copurchase_1052','MATCH (i:Item)<--(c:Customer) where c.louvainCommunity = 1052 RETURN DISTINCT id(i) as id','MATCH (i1:Item)<--(c:Customer)-->(i2:Item) WHERE c.louvainCommunity = 1052 RETURN distinct id(i1) as source, id(i2) as target, count(c) as weight');
