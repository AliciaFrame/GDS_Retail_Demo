...
LOAD CSV WITH HEADERS FROM "file:///UniqueCategories.csv"
WITH row.ITEMCATEGORY as ItemCategory
MERGE (c:Category{Category:ItemCategory})
RETURN COUNT (c);

LOAD CSV WITH HEADERS FROM "file:///UniqueItems.csv"
WITH toInteger(row.StockCode) as StockCode, row.Description as Description
MERGE (i:Item{StockCode: StockCode, Description:Description})
RETURN COUNT (i);

LOAD CSV WITH HEADERS FROM "file:///UniqueCountries.csv"
WITH row.Country as CountryName
MERGE (c:Country{Country:CountryName})
RETURN COUNT (c);

LOAD CSV WITH HEADERS FROM "file:///UniqueHouseholds.csv"
WITH toInteger(row.CustomerID) as CustomerID
MERGE (c:Customer{CustomerID:CustomerID})
RETURN COUNT (c);

LOAD CSV WITH HEADERS FROM "file:///UniqueTransactions.csv"
...
WITH row.StockCode as StockCode, row.CATEGORY as Category
...
WITH toInteger(row.NumberPurchased) as NumberPurchase, toInteger(row.CustomerID) as CustomerID, row.StockCode as StockCode
MATCH (c:Customer{CustomerID:CustomerID})
MATCH (i:Item {StockCode:StockCode})
...
WITH row.StockCode as StockCode, toFloat(row.Price) as Price, toInteger(row.Transaction_ID) as TransactionID, toInteger(row.Quantity) as Quantity
MATCH (i:Item{StockCode:StockCode})
MATCH (t:Transaction{TransactionID:TransactionID})
